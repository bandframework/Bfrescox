...
&FRESCO hcm=0.05 rmatch=20.0
 ...
 &PARTITION namep='projectile' massp=MASS_P zp=CHARGE_P
            namet='target'   masst=MASS_T zt=CHARGE_T qval=0.0 nex=1  /
...
def generate_elastic_template(
    ...
    pi_t (bool): Parity of the target nucleus (True for positive, False for negative).
    ...
    pi_p (bool): Parity of the projectile nucleus (True for positive, False for negative).
    ...
    replacements = {
        ...
        "PARITY_P": str((-1) ** float(pi_p + 1)),
        "MASS_T": str(mass_t),
        "CHARGE_T": str(charge_t),
        "PARITY_T": str((-1) ** float(pi_t + 1)),
