...
Test tolerances 
----------------
Reference results for tests are pickled fort.16 files generated by the standard Frescox. Absolute and relative tolerances are both set to `1e-16`.

Bfrescoxpro MPI and OpenMP testing
---------------------------------

Each test manifest includes a `ProSetup` field, e.g.:
...
which specifies the number of OpenMP threads and MPI processes to use when running Bfrescoxpro for that test. At least one test in each test suit should include a non-trivial number of OpenMP threads and MPI processes to ensure that the testing covers the parallel functionality of Bfrescoxpro.
